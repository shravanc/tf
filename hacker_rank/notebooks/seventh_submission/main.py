...
observe_file = "./observation_activation.csv"
of = open(observe_file, mode="w")
observe_writer = csv.writer(of, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
observe_writer.writerow(['batch_size', 'num_of_components', 'model_type', 'iteration', 'metric'])
...
for batch_size in batches:
    ...
    for nc in range(0, 8, 7):
        print(f"-->{nc}, {len(final_df)}")
        num_components = len(final_df.columns) - nc
        pca = PCA(n_components=num_components)
        ...
        # models = [base_model]
        for model_index, base_model in enumerate(models):
            model = base_model
            model.compile(
                loss=get_loss(),
                optimizer=get_optimizer(),
            ...
            for i in range(2):
                final_csv = f"./activation_test/b{batch_size}_c{nc}_m{model_index}_submission_{i}.csv"
                o_data = observe_writer.writerow([batch_size, nc, model_index, i, 0])
