...
# Normalize data
def norm(train, valid=None, test=None):
    stats = train.describe().transpose()
    train = (train - stats['mean']) / stats['std']
    # valid = (valid - stats['mean']) / stats['std']
    test = (test - stats['mean']) / stats['std']
...
def compile_and_fit(model, conf, train_dataset, valid_dataset, save_path):
    model.compile(
        loss=conf.loss,
...
def get_forecast(model_path, data):
